 ...
 * @description 100% 动态同步读取 stylemd/ 文件夹中 6 套精选排版主题 MD 文件与 SVG 开场动画指南的 Prompt 加载引擎
 */
...
/**
 * 100% 完整读取 stylemd/ 目录下对应的 MD 排版主题文件全量文本
 */
export function getThemePrompt(themeId: ThemeId): string {
  const filename = THEME_FILE_MAP[themeId] || THEME_FILE_MAP['moyu-green'];
...
/**
 * 动态读取 stylemd/theme-svg-cover-animation.md (首屏 SVG SMIL 动效指南)
 */
